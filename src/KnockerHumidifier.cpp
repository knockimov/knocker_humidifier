...
//const long intervalUpdate = (2 * 60 * 1000);     // interval at which to update the API (minutes * seconds/minute * milliseconds)
...
bool startConfiguration()
{
  ...
  WiFiManagerParameter custom_name("name", "KnockHum Name", htmlencode(my_name).c_str(),
                                   TKIDSIZE);
...
bool uploadData(uint8_t service)
{
  ...
  if (service == DTUbiDots)
  {
    ...
    sender.add("temperature", scaleTemperature(TEMPERATURE));
  ...
  {
    sender.add("T", scaleTemperature(TEMPERATURE));
...
void loop()
{
  ...
  buttonVal = digitalRead(BUTTON); // Read the state of the button
  ...
  { // Test for button release and store the up time
    if (ignoreUp == false)
    {
      char tempBuffer[TKIDSIZE];
      char humBuffer[TKIDSIZE];
      dtostrf(TEMPERATURE, 7, 2, tempBuffer);
      dtostrf(HUMIDITY, 7, 2, humBuffer);
      oledTimer = millis();
      Serial.println("");
      Serial.println("Printing to OLED screen");
      display.displayOn();
      display.clear();
#if defined(OLED32)
      display.setFont(Dialog_plain_6);
      display.drawString(10, 0, "TEMP");
      display.drawString(80, 0, "FUKT");
      display.setFont(Dialog_plain_10);
      display.drawString(10, 12, tempBuffer);
      display.drawString(80, 12, humBuffer);
      display.display();
#elif defined(OLED64)
      display.setFont(Dialog_plain_10);
      display.drawString(10, 0, "TEMP");
      ...
      display.drawString(10, 32, tempBuffer);
      display.drawString(80, 32, humBuffer);
      display.display();
#else
#error Unsupported display selected.
#endif
    }
    else
    {
    ...
    { // Test for button held down for longer than the hold time
      ...
      display.drawString(0, 11, "NAME:");
      display.drawString(0, 22, "FIRMWARE:");
      display.setFont(Dialog_plain_10);
      display.drawString(48, 0, WiFi.SSID());
      display.drawString(48, 11, my_name);
      display.drawString(48, 22, FIRMWAREVERSION);
      display.display();
